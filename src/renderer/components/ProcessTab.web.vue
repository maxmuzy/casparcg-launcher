    ...
    <b-row>
      <b-col>
        ...
        <b-form ref="commandForm" @submit.prevent="onSubmit" v-bind:class="{ hidden: !data.showCommandSend }">
          <b-input-group>
            <b-form-input placeholder="Enter a command (simulated)" id="command"></b-form-input>
...
export default {
  ...
  data() {
    return {
      data: this.$store.state.Process[this.$route.params.id] || { log: [], showCommandSend: false },
    }
  },
  ...
  watch: {
    $route(to) {
      ...
      this.data = this.$store.state.Process[to.params.id] || { log: [], showCommandSend: false }
    },
  },
  ...
  methods: {
    ...
    onSubmit(submitEvent) {
      ...
      this.$store.dispatch('logLine', {
        ...
        data: { type: 'event', content: '[Web Preview] Command sent: ' + command },
      })
